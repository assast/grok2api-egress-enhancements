...
质量守护页面复用管理员出口节点 API，固定管理 `grok_build` 作用域。页面支持新增、编辑、删除、启停、单节点手动质量检测、手动触发全量主动检测、节点列表刷新，以及单选、全选、批量启用、批量停用、批量连通检测和批量删除。批量连通检测按选择顺序逐个执行，不按出口 IP 去重；单节点失败不会中断整批，并返回成功/失败及 IP 变化汇总；代理地址保持只写；编辑时留空不会读取或清除现有代理。
