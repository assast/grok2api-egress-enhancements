...
- **账号表**：每账号的分类、Token/s、软/失败计数、动作、原因、最近观测时间与观测次数；每行「质量」按钮，多选支持批量检测。只列出 CPA 当前仍持有的账号，文件已被删除的账号直接从列表消失（历史仍保留在事件流里）
...
事件保留最近 **100** 条（环形缓冲，随状态文件持久化）。每条事件带 `source` 字段区分 `passive`（被动 usage）与 `probe`（主动探测），并带 `proxy` 字段记录该次请求实际走的出口代理：被动观测取账号当时的 `proxy_url` 绑定（未绑定记 `未绑定代理`），主动探测取探测请求真正使用的代理（无绑定且无环境代理时记 `直连`）。代理 URL 里的账号密码在写入事件前已被脱敏为 `***`。
